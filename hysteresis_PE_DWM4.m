...
function feloop = hysteresis_PE_DWM4(ammeter_obj, Loop_opts, fig, inverse)
amp = Loop_opts.amp;
...
if DWM_type
    ...
    if ~inverse
        feloop = measure_pos_part(obj, WF_str, draw_cmd, amp, Draw_obj, ...
            divider, delay, refnum, feloop);
        feloop = measure_neg_part(obj, WF_str, draw_cmd, amp, Draw_obj, ...
            divider, delay, refnum, feloop);
    else
        feloop = measure_neg_part(obj, WF_str, draw_cmd, amp, Draw_obj, ...
            divider, delay, refnum, feloop);
    ...
    if ~inverse
        set_waveform(obj, WF_str, 'bipolar');
    else
        set_waveform(obj, WF_str, 'inverse_bipolar');
    end

    % UPDATE
    [E_part, P_part] = measure_part(obj, draw_cmd, amp, Draw_obj, divider);
    feloop.E = E_part;
    feloop.P = P_part;
    feloop.type = "Classic";
end
